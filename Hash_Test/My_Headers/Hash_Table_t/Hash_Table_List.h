...
#ifndef HASH_TABLE_LIST_T_H
#define HASH_TABLE_LIST_T_H

#ifdef OK_DUMP
    #define EXTERNAL_OK_DUMP
#endif
#undef OK_DUMP
...
void list_destruct(List_t *list) {
    ...
    for (int i = list->head; i != 0 ; i = list->data[i].next) {
        free(list->data[i].value.key);
        free(list->data[i].value.value);
    }
    free(list->data);
    list->data = nullptr;
    list->size = 0;
...
int list_resize(List_t *list, size_t size, const char file[], const char function[], int line) {
    ...
    if(new_data == nullptr) {
        ...
        fclose(log);
        list_dump(list, ERR_STATE, "Allocation ERROR", file, function, line);
#endif
...
#ifdef EXTERNAL_OK_DUMP
    #define OK_DUMP
    #undef EXTERNAL_OK_DUMP
#endif

#endif //HASH_TABLE_LIST_T_H
